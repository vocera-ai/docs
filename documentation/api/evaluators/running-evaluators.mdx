...
async function runEvaluators(apiKey, evaluators, agentId = null, freq = 1) {
  const url =
    "https://api.cekura.ai/test_framework/v1/scenarios-external/run_scenarios/";
  ...
  if (typeof evaluators === "number" || typeof evaluators === "string") {
    if (!agentId) {
      throw new Error(
        "`agentId` is required when evaluators is number or string"
      );
    }
...
async function runEvaluators(apiKey, evaluators, agentId = null, freq = 1) {
  const url =
    "https://api.cekura.ai/test_framework/v1/scenarios-external/run_scenarios_text/";
